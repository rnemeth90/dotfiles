<#
    .SYNOPSIS
     Searches all Azure subscriptions in a tenant and prompts the user
     to remove empty resource groups
    .PARAMETER Mode
     The script currently accepts no parameters
    .EXAMPLE
     .\Clean-AzResourceGroups.ps1
    .NOTES
...
ForEach ($sub in $Subs) {
    ...
    foreach ($EmptyRG in $EmptyRGs){
        ...
        IF ($Confirmation -eq "y" -or $Confirmation -eq "Yes"){
            Write-Host "Deleting Resource Group:" $EmptyRG
